...
sampled <- sample(c(T,F), nrow(df_reg), replace = T, prob = c(0.7, 0.3))
...
```{r}
# Plot the covariates for the regressions
par(mfrow = c(1,2))

plot(df_reg$mean_ses, df_reg$PUNT_GLOBAL, pch = 16, cex = 0.5,
     main = "Scores vs peers SES",
     xlab = "peers SES", ylab = "Scores")
abline(model_peers, col = "red")

plot(df_reg$ESTU_INSE_INDIVIDUAL, df_reg$PUNT_GLOBAL, pch = 16, cex = 0.5,
     main = "Scores vs individual SES",
     xlab = "individual SES", ylab = "Scores")
abline(model_indiv, col = "red")
```
